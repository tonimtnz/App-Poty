...
export function ArtistSongCard({
  songName,
  songId,
  songReps,
  songImg,
  ...
  return (
    <div className="card-and-iframe">
      <a onClick={handleSong} className="artist-song-card">
        <img src={songImg} />
        <div className="artist-song-info">
          <h3>{songName}</h3>
          <h5>{songReps} Reproducciones</h5>
          <h5>{songTimeMins.toFixed()} Minutos Aprox.</h5>
        </div>
        {!togle && <button >
          <svg
            width="12"
      ...
      {togle && <iframe className="song-repro" style={{borderRadius: "12px", boxSizing: "border-box"}} src={`https://open.spotify.com/embed/track/${songId}?utm_source=generator`} width={"50%"} height={"100%"} frameBorder={"0"} allowfullscreen={""} allow={"autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"} loading={"lazy"}></iframe>}
    </div>
  );
}
